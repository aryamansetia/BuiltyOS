...
### Option A (recommended): Deploy both frontend and backend on Render

This repo now includes a Render blueprint file: `render.yaml`.

#### 1. Prepare production database

- Create a MongoDB Atlas cluster (or any hosted MongoDB instance).
- Keep connection string ready for `MONGO_URI`.
...
4. Render will detect `render.yaml` and create:
  - `builtyos-backend` (Node web service)
  - `builtyos-frontend` (static site)
5. Before final deploy, set backend secret env var:
  - `MONGO_URI=<your production mongo uri>`
6. Deploy services.

#### 3. Verify production environment values

- Backend env:
  - `NODE_ENV=production`
  - `JWT_SECRET` (auto-generated by Render)
  - `JWT_EXPIRES_IN=7d`
  - `FRONTEND_URL=https://builtyos-frontend.onrender.com`
  - `GPS_SIMULATION_INTERVAL_MS=20000`
- Frontend env:
  - `VITE_API_URL=https://builtyos-backend.onrender.com/api`

If you rename services, update both URLs accordingly.

#### 4. Health check

- Backend health endpoint: `https://<your-backend-domain>/api/health`

### Option B: Backend on Render + Frontend on Vercel

Frontend includes SPA routing config in `frontend/vercel.json`.

1. Deploy backend first (Render web service, root `backend/`).
2. Deploy frontend in Vercel (root directory `frontend/`).
3. Set Vercel env variable:
  - `VITE_API_URL=https://<your-render-backend-domain>/api`
4. Redeploy frontend after setting env.

### Production notes

- Do not run `npm run seed` against production unless you intentionally want to reset data.
- CORS is controlled by backend env variable `FRONTEND_URL`.
- API examples for smoke testing are available in `docs/api-samples.http`.
